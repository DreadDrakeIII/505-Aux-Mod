	...
	if (_curTime > 20) then {
		...
		if (_curTime > 10) then {
			...
			if ((time - _lastBeepTime) >= 0.5) then {
				playSound3D ["\BLU\OLI\addons\zeus\minesweeper\sounds\standard_beep.ogg", _ied, false, getPosASL _ied, 5, 1, 150];
				_lastBeepTime = time;
			};
		} else {
			if (_curTime > 0) then {
				// Very fast beeping - every 0.25 second
