fn_goko_ballistic_impact_main =
{
    ...
    //   1 = players get NOTHING (AI-only mode)
    //   2 = players get HELMET destruction only (no facewear/NVG strip, no gore/blood)
    // AI (non-players) always run the full path.
    private _isPlayerUnit = isPlayer _unit;
    private _playerMode = if (isNil "gokobi_var_playerMode") then { 0 } else { gokobi_var_playerMode };

    if (_isPlayerUnit && _playerMode == 1) exitWith {};

    private _helmetOnly = _isPlayerUnit && _playerMode == 2;
    // ------------------------------------------------------------------------

    if (!_helmetOnly && _getGlasses != "") then
    {
        removeGoggles _unit;
        _unit remoteExecCall ["gokoBI_particleFX_brokenGlassParticles"];
        _unit remoteExecCall ["gokoBI_particleFX_smallHelmetParts"];
    };
    if (!_helmetOnly && _getHeadMountedDisplay != "") then
    {
    ...
    if (_headgearProtection != 0) then
    {
        ...
        if ( floor _projectileForce > _headgearProtection && _projectileCaliber >= gokobi_var_AllowedCaliber) then
        {
            ...
            if (!_helmetOnly) then { _unit remoteExecCall [_bloodParticleEffects] };
            ...
            {
                if (!_helmetOnly) then { _unit remoteExecCall [_goreParticleEffects] };
    ...
    else
    {
        ...
        if (!_helmetOnly) then { playsound3d [format ["%1",_headshotFleshEffects], _unit, false, getPosASL _unit, 1, 1, 70] };
        if (!_helmetOnly) then { _unit remoteExecCall [_bloodParticleEffects] };
        removeHeadgear _unit;
    };
};

if (gokobi_var_debug) then {
    diag_log "[GOKO Player Patch] fn_goko_ballistic_impact_main override applied.";
};
